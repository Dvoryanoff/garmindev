...
- отдельная служебная админка теперь доступна через `Django Admin`
...
- отдельная `Django Admin` для управления пользователями, загрузками, job-ами и последними отчётами
...
Вместе с ним автоматически поднимается `Django Admin` на [http://127.0.0.1:8010/admin/](http://127.0.0.1:8010/admin/).
...
Bootstrap-login для локальной `Django Admin` по умолчанию:
...
Служебная админка теперь работает через `Django Admin`.

Локально:

- dashboard: [http://127.0.0.1:8000](http://127.0.0.1:8000)
- Django admin: [http://127.0.0.1:8010/admin/](http://127.0.0.1:8010/admin/)

При запуске проекта:

- автоматически применяются Django migrations
- автоматически создаётся bootstrap-superuser для Django admin, если его ещё нет

В Django admin доступны:

- `Пользователи Garmin`
- `Загруженные FIT-файлы`
- `Фоновые задачи`
- `Помесячная история`
- `Последние отчёты`

Через неё сейчас можно:

- смотреть список пользователей
- менять роль `user/admin`
- включать и отключать учётные записи
- смотреть uploads и background jobs
- смотреть последние `10` сохранённых отчётов по каждому пользователю
- смотреть накопленную `monthly_history`

...
- читает локальные FIT-файлы из папки `resources/` и её подпапок
...
При старте dashboard проект обновляет набор накопительных Excel-файлов в `monthly_history/`.
Для каждого пользователя создаётся свой файл с листами по годам.

Формат листов:

- лист = год
- первая колонка: `Дата` в формате `Январь 2024`
- дальше только темпы по фиксированным дистанциям
...
Для monthly history используется реестр уже обработанных тренировок:

- пользователь определяется в первую очередь по `serial_number`
- имя файла пользователя берётся из имени FIT-файла, если оно там есть
- тренировка считается уникальной по `activity_key`, а не по имени файла
- если один и тот же workout встретился повторно под другим именем файла, второй раз он не попадёт в monthly history
